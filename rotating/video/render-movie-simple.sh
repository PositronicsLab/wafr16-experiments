...
usage()
{
  echo "render_movie_simple.sh <frame rate> <path to Moby render> <camera position> <path to osg files/output> <camera target> <movie file> <up vector>"
...
for i in $6/driver.out-*.osg; do
  ...
  [ -f $6/$new ] && echo "Already created $new" || 
moby-render $i -p $3 $4 $5 -t $7 $8 $9  -u ${11} ${12} ${13} $6/${new};
  let a=a+1;
done
